  ...
  HugoAlgolia.prototype.formatDataInFile = function(meta, filePath) {
    ...
    if (meta.data) {
      for (prop in meta.data) {
        ...
        let content = stopword.removeStopwords(
          meta.content.split(/\s+/).join(" ")
        );
        let truncatedContent = truncate(content, MAX_CONTENT_SIZE); // 20kB limit
