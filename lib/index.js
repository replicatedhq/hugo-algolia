...
function HugoAlgolia(input, output, index) {
    ...
    HugoAlgolia.prototype.index = function() {
        ...
        if(self.multInd) {
            if(self.indexById) self.writeSecondIDList();
    ...
    HugoAlgolia.prototype.writeSecondIDList = function() {
    ...
    HugoAlgolia.prototype.pushToIndices = function(name, obj, objCuid) {
        let indices = self.indices;
        let isIndexInIndices = indices.hasOwnProperty(name);
        let itemToStore = self.indexById ? objCuid : item;
            ...
            if(hasIndex) {
                ...
                if(_.isArray(indexProp)) {
                    for (item of indexProp) {
                        ...
                        self.pushToIndices(indexName, obj, objCuid);
                    }
                } else {
                ...
                self.pushToIndices(other, obj, objCuid);
            }
        }
